...
<body>
	...
	<div class= "content">
		...
		<div class= "qlist-wapper">
			...
			<?php
				$rows = $db->query("SELECT q.id, title, time, name FROM question q JOIN user u JOIN tag t JOIN tag_question tq ON q.u_id = u.id and t.id = tq.t_id and q.id = tq.q_id WHERE title like \"%$keyword%\" or t.name like \"$keyword%\" ORDER BY pinned DESC, time DESC");
				foreach ($rows as $row) {
			?>
			<div class= "question">
				<div class= "question-num-summary">
